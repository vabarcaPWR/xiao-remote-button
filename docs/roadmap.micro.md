...
### Micro: Support P0.02 (D0) + P1.15 (D10) simultaneously
...
- [x] Add second relay alias (`relay1`) in board overlay: `P0.10 GPIO_ACTIVE_HIGH`
- [x] Update `relay_hal.c` to initialize and drive both pins simultaneously
- [x] Both pins set HIGH on `relay_on()`, both LOW on `relay_off()`
- [x] If one pin fails init, system still operates with the other (graceful degradation)
- [x] GPIO default LOW for both pins at boot (fail-safe preserved)
- [x] Ceedling unit tests: relay module tests pass (HAL is mocked; dual-pin is hardware-validated)
- [x] Update Hardware Pin Assignment table in docs
...
| Relay MOSFET gate (secondary) | D10 | P1.15 | Active HIGH → relay ON (next to 3.3V) |

### 🔄 Cross-validation ✅
- [x] **Physical check**: Multimeter on both P0.02 and P1.15 confirms identical voltage change
